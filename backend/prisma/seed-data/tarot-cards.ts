...
import type { TarotSuit } from "@prisma/client";

type TarotCardSeed = Prisma.TarotCardCreateManyInput;

const majorArcana: Array<{ number: number; nameEn: string; nameKo: string }> = [
...
const suits: Array<{ suit: TarotSuit; suitKo: string; suitEn: string; baseKey: number }> = [
...
const ranks: Array<{ rank: string; rankKo: string; rankEn: string; offset: number }> = [
...
export function buildTarotCardSeeds(): TarotCardSeed[] {
  const majors: TarotCardSeed[] = majorArcana.map((c) => ({
    nameEn: c.nameEn,
    nameKo: c.nameKo,
    arcana: "MAJOR",
    suit: null,
  ...
  const minors: TarotCardSeed[] = suits.flatMap(({ suit, suitKo, suitEn, baseKey }) =>
    ranks.map((r) => ({
